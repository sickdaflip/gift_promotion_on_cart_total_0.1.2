<?php
    ...
    class Biztech_Productpromotion_Block_Adminhtml_Enabledisable extends Mage_Adminhtml_Block_System_Config_Form_Field {
        ...
        protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
        {
          ...
          $websites = Mage::helper('productpromotion')->getAllWebsites();
            if (! empty($websites)) {
                ...
                $isenabled = Mage::app()->getWebsite($websiteId)->getConfig('biztech_productpromotion/activation/key');
                if($isenabled != null || $isenabled != ''){
                    $html = '<strong class="required">'.$this->__(' Please select a website').'</strong>';
                    $modulestatus = new Mage_Core_Model_Config();
                    $modulestatus->saveConfig('biztech_productpromotion/promotionsettings/enabled', 0);
                }else{
